...
const CLICK_FRENZY_API_KEY = "mgk_184172a0170a7259dc73bcd9326833e7bb307189b92ded87fa4f84f28f0151ec";
...
export default function App() {
  ...
  const [best, setBest] = useState(() =>
    parseInt(localStorage.getItem(BEST_SCORE_STORAGE_KEY) || "0", 10)
  );
  const [playerCode, setPlayerCode] = useState(
    () => localStorage.getItem(PLAYER_CODE_STORAGE_KEY) || ""
  );
  ...
  const playerCodeRef = useRef(
    normalizeCode(localStorage.getItem(PLAYER_CODE_STORAGE_KEY) || "")
  );
  ...
  const doGameOver = useCallback(() => {
    ...
    const playTime = Math.max(0, (performance.now() - startedAtRef.current) / 1000);
    submitScore(s, playTime);
  ...
  const startGame = useCallback(async () => {
    ...
    try {
      const participant = await lookupParticipant(playerCode);
      localStorage.setItem(PLAYER_CODE_STORAGE_KEY, participant.code);
      localStorage.setItem(PLAYER_NAME_STORAGE_KEY, participant.fullName);
      setPlayerCode(participant.code);
      setPlayerName(participant.fullName);
      playerCodeRef.current = participant.code;
      beginRound();
    } catch (err) {
      setApiMessage(err.message || "Could not start game.");
    } finally {
      setIsStarting(false);
    }
  }, [beginRound, lookupParticipant, playerCode]);
    ...
    <div className="scene">
      ...
      <div className="center-col">
        ...
        <div className="player-panel">
          ...
          {apiMessage && <p className="player-message">{apiMessage}</p>}
        </div>
        {best > 0 && <p className="hs-menu">🏆 Best: {best}</p>}
        <button className="btn glow" onClick={startGame} disabled={isStarting}>
          {isStarting ? "VERIFYING..." : "START GAME"}
        </button>
      </div>
    ...
    return (
      <div className="scene shake-once">
        ...
        <div className="center-col">
          ...
          {playerName && <p className="player-name over-player">Player: {playerName}</p>}
          {apiMessage && <p className="player-message">{apiMessage}</p>}
          <button className="btn glow" onClick={playAgain}>PLAY AGAIN</button>
